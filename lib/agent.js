'use strict';
    ...
    async function runOAIAgent(apiUrl, keys, models, question, history, systemPrompt, onToolCall, keyCounter, overrideSystemPrompt) {
    if (overrideSystemPrompt) systemPrompt = overrideSystemPrompt;
    ...
    if (GROQ_KEYS.length) {
      try {
        const text = await runOAIAgent(
          ...
          GROQ_KEYS, GROQ_MODELS, question, history, systemPrompt, onToolCall, _groqIdx++, overrideSystemPrompt,
        );
        return { text, sources: [] };
      } catch (e) {
        lastErr = e;
    ...
    if (OPENAI_KEYS.length) {
      try {
        const text = await runOAIAgent(
          ...
          OPENAI_KEYS, OPENAI_MODELS, question, history, systemPrompt, onToolCall, _openaiIdx++, overrideSystemPrompt,
        );
        return { text, sources: [] };
      } catch (e) {
        lastErr = e;
