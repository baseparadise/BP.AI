...
async function handleChartCommand(message, symbol, interval, candles) {
  try {
    ...
    // chart image max 500 candle agar tetap terbaca; analisis teks pakai displayN penuh
    ...
    let text = '**📊 '+pair+' | '+interval.toUpperCase()+' | '+actualN+' candle (chart: '+chartNActual+')** '+chgEmoji+' '+chgStr+'\n';
    ...
    let file = null;
    try {
      const buf = await buildChart(pair,ohlcv,sma20,sma50,bb,rsi,macdData,interval,candlePatterns,complexPatterns,chartN);
