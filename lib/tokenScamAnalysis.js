...
async function getLpLockInfo(pairAddress, chain, clankerData, bestPair, tokenAddr) {
  ...
  if (pairAddress && ESCAN_KEY && !result.lpLocked) {
    ...
    if (!Array.isArray(lpHolders) || lpHolders.length === 0) {
      // tokenholderlist gagal atau kosong (normal untuk V3 pool — LP adalah NFT, bukan ERC-20)
      if (result.dexType === 'v3') {
        result.notes.push('V3 pool: LP bukan ERC-20 — tokenholderlist tidak berlaku. Gunakan Layer C (NFT check)');
      } else {
        result.notes.push('tokenholderlist kosong atau gagal — tidak ada LP holder data');
      }
      if (!result.lpLocked) result.lpLockMethod = result.lpLockMethod || 'unconfirmed';
    } else if (Array.isArray(lpHolders)) {
      ...
      if (!result.lpLocked && !lockerFound && lpHolders.length > 0) {
...
async function collect(addresses, chain) {
  ...
  for (const addr of addresses.slice(0,2)) {
    ...
    const lpStatus = !lpLock ? '❌ N/A'
      ...
      : lpLock.lpLockMethod === 'no_apikey' ? '❓ Butuh ETHERSCAN_API_KEY (tidak ada di env)'
      : lpLock.lpLockMethod === 'no_pair' ? '❓ Pair address tidak ditemukan'
      : lpLock.lpLockMethod === 'unconfirmed' ? '❓ Tidak terkonfirmasi (' + (lpLock.dexType||'?') + ' pool)'
      : '❓ Tidak bisa dikonfirmasi';
    lines.push(`  LP Lock check:    ${lpStatus}`);
