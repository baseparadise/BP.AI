...
const SYSTEM_PROMPT_BASE = [
  ...
  '⚠️ ATURAN FORMAT DISCORD YANG WAJIB: (1) DILARANG KERAS menggunakan tabel markdown (baris dengan karakter pipe | seperti | kolom1 | kolom2 |) — Discord tidak bisa merender tabel, yang muncul hanya simbol | yang mengganggu. (2) DILARANG menggunakan tag HTML seperti <br> atau <b> — tidak tampil di Discord. (3) Untuk perbandingan atau daftar multi-item, WAJIB gunakan format bullet point dengan bold: "• **Nama:** deskripsi" dipisah newline. (4) Untuk perbandingan beberapa entitas (misal Binance vs Coinbase vs Kraken), bahas SATU PER SATU secara berurutan, bukan dalam tabel.',
  ...
  'PANJANG JAWABAN: Untuk pertanyaan teks biasa (bukan kode), targetkan 1200-1800 karakter — isi penuh satu pesan Discord. Ringkas tapi padat: utamakan poin paling penting, buang pengulangan dan basa-basi. Jangan terlalu pendek (< 400 karakter untuk pertanyaan substantif). Jika ada daftar panjang, pilih 3-5 item terpenting. Jangan melebihi 1900 karakter karena Discord membatasi 2000 karakter per pesan.',
].join(' ');
  ...
  while (i < lines.length) {
    ...
    if (/^\s*\|/.test(line)) {
      ...
      if (header.length === 0 || rows.length === 0) {
        ...
        // 3+ kolom: header jadi label bold, tiap baris jadi entry multi-field
        for (const row of rows) {
          const parts = header.map((h, idx) => {
            const val = row[idx] || '—';
            return idx === 0 ? `**${val}**` : `${h}: ${val}`;
          });
...
function formatAnswer(text, sources = []) {
  ...
  if (sources.length > 0) {
    ...
    const footer = `\n\n📚 **Sumber:**\n${list}`;
    const maxBody = 8000 - footer.length;
    if (out.length > maxBody) out = out.slice(0, maxBody - 1) + '…';
    out += footer;
  } else if (out.length > 8000) {
    // Safety net — bot.js akan split jika perlu
    out = out.slice(0, 7999) + '…';
  }
  return out;
}
