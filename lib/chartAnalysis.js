...
async function buildChart(pair, ohlcv, sma20, sma50, bb, interval) {
  ...
  function fmtL(t) {
    const d = new Date(t);
    const mo = (d.getMonth()+1).toString().padStart(2,'0'), dy = d.getDate().toString().padStart(2,'0'), h = d.getHours().toString().padStart(2,'0');
  ...
  const resp = await axios.post('https://api.quickchart.io/chart', body, {
    responseType:'arraybuffer', timeout:20000, headers:{ 'Content-Type':'application/json' },
  });
