...
async function getLpLockInfo(pairAddress, chain, clankerData, bestPair) {
  const result = {
    lpLocked:      false,
    ...
    if (nfpmAddr) {
      ...
      if (tokenIds.length > 0) {
        ...
        const posResults = await Promise.all(tokenIds.map(async(tid) => {
          ...
          if (!raw) return null;
          const h = raw.startsWith('0x') ? raw.slice(2) : raw;
          // Returns: nonce(32) operator(32) token0(32) token1(32) fee(32) ...
          if (h.length < 128) return null;
          const token0 = '0x' + h.slice(64+24, 64+64).toLowerCase();
          const token1 = '0x' + h.slice(128+24, 128+64).toLowerCase();
          return { tid, token0, token1 };
        }));
        const matched = posResults.filter(p => p && (
          p.token0.includes(pairAddress.toLowerCase().slice(2)) ||
          p.token1.includes(pairAddress.toLowerCase().slice(2))
        ));
        if (matched.length > 0 || tokenIds.length > 0) {
          result.lpLocked     = true;
          ...
          result.notes.push(`V3 NFT check: ${tokenIds.length} posisi LP di burn address (NFPM ${nfpmAddr.slice(0,8)}…)`);
          result.notes.push(`tokenId(s): ${tokenIds.slice(0,3).join(', ')}`);
        }
      } else if (ESCAN_KEY) {
...
async function collect(addresses, chain) {
  ...
  for (const addr of addresses.slice(0,2)) {
    ...
    const [allData, isVerified, totalSupply, owner, deployer, offChain, lpLock] = await Promise.all([
      ...
      getLpLockInfo(bestPair?.pairAddress || null, chain, clankerData, bestPair),
    ]);
