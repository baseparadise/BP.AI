...
// === OPENAI ===
const OPENAI_MODELS = [...new Set(parseEnvList(process.env.OPENAI_MODELS, [
  'gpt-4o-mini',
  'gpt-5-mini',
  'gpt-3.5-turbo',
]))];
...
// [FIX] Default: Gemini duluan karena context window lebih besar dari Groq free tier.
// Groq free tier hanya ~6.000 token per request → mudah 413 kalau ada file besar.
// Override dengan env AI_PROVIDER_ORDER=groq,gemini jika ingin Groq duluan.
const PROVIDER_ORDER = parseEnvList(process.env.AI_PROVIDER_ORDER,
  GEMINI_KEYS.length > 0 ? ['gemini', 'groq', 'openai'] : ['groq', 'gemini', 'openai']
);
...
async function callProviderOnce(providerId, opts, timeout, rounds = 3) {
  ...
  let lastErr;
  // [FIX] Flag untuk langsung keluar dari semua loop jika request terlalu besar.
  // 413 request_too_large = ukuran payload melebihi batas provider, bukan rate limit.
  // Tidak ada gunanya retry dengan key lain karena ukuran request tidak berubah.
  ...
  for (let round = 0; round < rounds; round++) {
    ...
    for (const model of provider.models) {
      ...
      for (let i = 0; i < provider.keys.length; i++) {
        ...
        try {
          ...
          const keyIdxOk = (keyCursors[providerId] - 1) % provider.keys.length;
          console.log(`[${providerId}] ✅ ok model=${model} key#${keyIdxOk}`);
          ...
          // [FIX] 413 request_too_large: request terlalu besar untuk model/provider ini.
          // Retry dengan key lain tidak akan membantu — langsung pindah ke provider berikutnya.
          if (status === 413 && (apiCode === 'request_too_large' || /request.*too.*large|payload.*too.*large/i.test(apiMsg))) {
            console.log(`[${providerId}] model=${model} key#${keyIdx} REQUEST TERLALU BESAR (413), skip provider ini langsung.`);
            requestTooLarge = true;
            break;
          }

          // [FIX] 413 rate_limit_exceeded di Groq = juga berarti request terlalu besar untuk free tier.
          // Meskipun namanya "rate limit", errornya disebabkan token payload yang terlalu besar.
          if (status === 413) {
            console.log(`[${providerId}] model=${model} key#${keyIdx} 413 (payload terlalu besar untuk free tier), skip semua model di provider ini.`);
            requestTooLarge = true;
            break;
          }

          if (status === 403 && (apiCode === 'model_permission_blocked_org' || /blocked at the organization|permission/i.test(apiMsg))) {
            console.log(`[${providerId}] model=${model} key#${keyIdx} DIBLOKIR org-level, skip model ini.`);
          ...
          if (RETRYABLE_STATUS.includes(status)) {
            ...
            console.log(`[${providerId}] model=${model} key#${keyIdx} timeout/network, coba kombinasi lain...`);
          } else {
            console.log(`[${providerId}] model=${model} key#${keyIdx} status ${status} (${apiCode || ''}), coba kombinasi lain...`);
          }
          continue;
        }
      }
    ...
    if (round < rounds - 1) {
      ...
      console.log(`[${providerId}] semua model & key sibuk, backoff ${Math.round(waitTime)}ms (putaran ${round + 1}/${rounds})...`);
...
async function callGemini(models, body, timeout, rounds = 4) {
  ...
  let lastErr;
  let imgKeyCursor = keyCursors.gemini || 0;
  for (let round = 0; round < rounds; round++) {
    ...
    console.log(`[callGemini/image] semua model & key sibuk, backoff ${Math.round(waitTime)}ms (putaran ${round + 1}/${rounds})`);
    await sleep(waitTime);
  }
  keyCursors.gemini = imgKeyCursor;
  throw lastErr;
...
module.exports = {
  SYSTEM_PROMPT, GEMINI_MODEL, TEXT_MODELS, IMAGE_MODELS,
