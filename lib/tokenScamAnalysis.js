...
const CHAIN = {
  ...
  solana:   { name:'Solana',      cid:null,  gid:'solana',      did:'solana',    ex:'https://solscan.io' },
...
async function collect(addresses, chain) {
  ...
  for (const addr of addresses.slice(0,2)) {
    ...
    if (isSolana) {
      ...
      if (solPair) lines.push('  DEX: ' + (solPair.dexId||'?') + ' | Liq: 
      // LP lock status
    ...
    if (isSolana) {
      lines.push('  ⛔ Solana — gunakan Solscan.io atau SolanaFM untuk info deployer');
    } else {
    ...
    if (holders?.length) {
      ...
      lines.push(isSolana ? '  ⛔ Solana — gunakan Solscan.io/richlist untuk holder distribution' : '  N/A (perlu ETHERSCAN_API_KEY)');
    }
    ...
    // ── Token Info verbatim block — AI HARUS salin persis, tidak boleh ubah apapun
    {
      ...
      const _platform = isSolana ? 'Solana SPL Token' : (allData?.context || (clankerData?.found ? 'Clanker' : 'tidak terdeteksi'));
      const _sym  = gecko?.symbol || bestPair?.baseToken?.symbol || '-';
      const _name = gecko?.name   || bestPair?.baseToken?.name   || 'tidak diketahui';
      const _verified = isSolana ? 'ℹ️ N/A (Solana)'
        : isVerified === true  ? '✅ Ya (clanker.world)'
...
module.exports = { isScamAnalysisRequest, runScamAnalysis };
 + (parseFloat(solPair.liquidity?.usd||0).toLocaleString()));
    } else if (!lpLock) {
