...
// Cache redirect destinations agar tiap endpoint hanya redirect 1x seumur proses
const redirectCache = {};
...
function zerionGet(path, params) {
  return new Promise(async (resolve, reject) => {
    ...
    // Gunakan cached redirect jika sudah pernah di-resolve sebelumnya
    ...
    for (let hop = 0; hop <= maxRedirects; hop++) {
      ...
      // Follow redirect & simpan ke cache agar request berikutnya langsung ke tujuan
      if (status >= 300 && status < 400) {
        ...
        currentPath = location.startsWith('http')
          ...
          : location;
        // Simpan mapping original → final path
        redirectCache[originalPath] = currentPath;
        continue;
      }
      ...
      if (status === 429)
        return reject(new Error('RATE_LIMIT'));  // kode khusus agar retry bisa skip
      if (status === 404)
      ...
      if (!ct.includes('application/json') && !ct.includes('application/vnd.api+json')) {
        return reject(new Error(
          ...
          'Kemungkinan API key salah. Body: ' + body.slice(0, 120)
        ));
      }
...
async function zerionGetRetry(path, params, maxRetries) {
  const tries = maxRetries || 2;  // turunkan default retry jadi 2 hemat quota
  for (let i = 0; i < tries; i++) {
    try {
      ...
      // Jangan retry jika rate limit — langsung lempar error asli
      if (e.message === 'RATE_LIMIT') throw new Error('Rate limit Zerion. Coba lagi sebentar.');
...
async function fetchZerionPortfolio(address) {
  ...
  const positions = positionsRes.status === 'fulfilled'
    ...
    : [];
  const nftMeta   = nftRes.status === 'fulfilled' ? nftRes.value?.meta ?? null : null;
  const pnl       = pnlRes.status === 'fulfilled' ? pnlRes.value?.data?.attributes ?? null : null;
