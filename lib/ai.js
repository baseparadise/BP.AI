...
const SYSTEM_PROMPT_BASE = [
  ...
  // Format Discord — larang tabel markdown
  'PENTING: Jangan gunakan format tabel markdown (baris dengan karakter | pipe) karena Discord TIDAK bisa menampilkan tabel. Gunakan bullet points, bold teks, atau daftar bernomor sebagai gantinya. Juga jangan gunakan tag HTML seperti <br> karena tidak dirender di Discord.',
...
function convertTablesForDiscord(text) {
  ...
  // Hapus tag HTML <br>, <br/>, <br /> → newline
  text = text.replace(/<br\s*\/?>/gi, '\n');

  // Konversi blok tabel markdown ke format teks yang bisa dibaca di Discord
  // Deteksi blok tabel: rangkaian baris yang dimulai dengan |
  text = text.replace(/((?:\|[^\n]+\|\n?)+)/g, (tableBlock) => {
    const lines = tableBlock.trim().split('\n').map(l => l.trim()).filter(Boolean);
    if (lines.length < 1) return tableBlock;

    // Parse baris header
    const parseRow = (line) => line.split('|').map(c => c.trim()).filter(Boolean);
    const header = parseRow(lines[0]);

    // Cek apakah baris kedua adalah separator (---|---)
    let dataStart = 1;
    ...
    // Format: 2 kolom → "**Col1** — Col2" per baris (key-value style)
    if (header.length === 2) {
    ...
    // Format: 3+ kolom → per-baris jadi beberapa bullet
    for (const row of rows) {
      const parts = header.map((h, i) => `**${h}:** ${row[i] || '—'}`);
