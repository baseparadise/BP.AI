...
const PROVIDERS = {
  ...
  conduit:{
    ...
    parseResponse(d){const c=d?.choices?.[0];if(c?.finish_reason==='length')console.warn('[conduit] ⚠️ Terpotong.');return{text:c?.message?.content||'Tidak ada jawaban.',sources:[]};},
  },
};
    ...
    for (const model of provider.models) {
      ...
      try {
        ...
        if (isAIMetaError(parsed.text)) {
          console.warn(`[${providerId}] ⚠️ key#${keyIdx} model=${model} mengembalikan meta-error Claude, retry ke model/provider berikutnya...`);
