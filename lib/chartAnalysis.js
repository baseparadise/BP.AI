...
// Kraken hanya punya interval: 1,5,15,30,60,240,1440,10080,21600 menit
...
function detectPatterns(ohlcv) {
  ...
  if (r > 0 && bd/r < 0.1) out.push({ label:'Doji', emoji:'🔸', desc:'Ketidakpastian — potensi pembalikan arah' });
  ...
  if (isBear(prev) && isBull(last) && last.c > prev.o && last.o < prev.c) out.push({ label:'Bullish Engulfing', emoji:'📗', desc:'Candle hijau menelan candle merah — reversal naik' });
  else if (isBull(prev) && isBear(last) && last.c < prev.o && last.o > prev.c) out.push({ label:'Bearish Engulfing', emoji:'📕', desc:'Candle merah menelan candle hijau — reversal turun' });
  if (isBear(prev2) && body(prev) < body(prev2)*0.3 && isBull(last) && last.c > (prev2.o+prev2.c)/2) out.push({ label:'Morning Star', emoji:'🌟', desc:'Reversal bullish 3 candle — sinyal kuat naik' });
  else if (isBull(prev2) && body(prev) < body(prev2)*0.3 && isBear(last) && last.c < (prev2.o+prev2.c)/2) out.push({ label:'Evening Star', emoji:'🌆', desc:'Reversal bearish 3 candle — sinyal kuat turun' });
...
// Candlestick chart via QuickChart v4 (chartjs-chart-financial)
// PENTING: wajib version:4 agar QuickChart support tipe 'candlestick'
async function buildChart(pair, ohlcv, sma20, sma50, bb, interval, patterns) {
  ...
  // Label pola yang terdeteksi untuk judul chart
  const patternStr = patterns && patterns.length > 0
    ? ' | ' + patterns.map(p => p.emoji + p.label).join(', ')
    : '';
  ...
  const cfg = {
    ...
    data: {
      datasets: [
        {
          label: pair,
          data: sl.map(c => ({ x: c.t, o: c.o, h: c.h, l: c.l, c: c.c })),
          color: {
        ...
        },
        {
          type: 'line',
          label: 'BB Upper',
          data: sl.map((c, i) => ({ x: c.t, y: bbs[i].upper })),
          borderColor: 'rgba(206,147,216,0.7)',
          borderWidth: 1,
          pointRadius: 0,
          fill: false,
        },
        {
          type: 'line',
          label: 'BB Lower',
          data: sl.map((c, i) => ({ x: c.t, y: bbs[i].lower })),
          borderColor: 'rgba(206,147,216,0.7)',
          borderWidth: 1,
          pointRadius: 0,
          fill: false,
    ...
    options: {
      plugins: {
        ...
        title: {
          display: true,
          text: pair + ' | ' + interval.toUpperCase() + ' — Candlestick / SMA20 / SMA50 / BB' + patternStr,
          color: '#EEE',
  ...
  try {
    ...
    if (e.response && e.response.data) {
      const errBody = Buffer.isBuffer(e.response.data)
        ? e.response.data.toString('utf8')
        : String(e.response.data);
      const errHeader = e.response.headers && e.response.headers['x-quickchart-error'];
      console.error('[chart] QuickChart error ('+e.response.status+'):', errHeader || errBody.slice(0, 200));
    }
    throw e;
  }
}
...
async function handleChartCommand(message, symbol, interval) {
  try {
    ...
    if (patterns.length > 0) {
      text += '\n**🕯️ Pola Candlestick:**\n';
      text += patterns.map(p => '• '+p.emoji+' '+p.label+' — '+p.desc).join('\n') + '\n';
    } else {
    ...
    let chartBuf = null;
    try {
      chartBuf = await buildChart(pair, ohlcv, sma20, sma50, bb, interval, patterns);
    } catch(e) {
