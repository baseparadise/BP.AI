...
// === KONFIGURASI GROQ ===
const DEFAULT_GROQ_MODELS = ['groq/compound', 'groq/compound-mini'];
const GROQ_MODELS = [...new Set(parseEnvList(process.env.GROQ_MODELS, DEFAULT_GROQ_MODELS))];
const GROQ_KEYS = (process.env.GROQ_API_KEYS || process.env.GROQ_API_KEY || '').split(',').map((k) => k.trim()).filter(Boolean);
let keyCursorGroq = GROQ_KEYS.length ? Math.floor(Math.random() * GROQ_KEYS.length) : 0;
...
console.log(`[AI] ${GROQ_KEYS.length} key Groq & ${API_KEYS.length} key Gemini dimuat.`);
...
// ==========================================
// PROVIDER GROQ (Utama) - Timeout 30 Detik
// ==========================================
async function callGroq(question, timeout = 30000) {
  if (GROQ_KEYS.length === 0) {
    throw new Error('GROQ_API_KEYS belum di-set.');
  }

  let lastErr;
  for (const model of GROQ_MODELS) {
    for (let i = 0; i < GROQ_KEYS.length; i++) {
      const key = GROQ_KEYS[keyCursorGroq % GROQ_KEYS.length];
      keyCursorGroq++;
      ...
      try {
        ...
        console.log(`[AI] Sukses menjawab via Groq (${model})`);
        return { text: data.choices[0].message.content, sources: [], provider: 'Groq' };
        
      } catch (err) {
        lastErr = err;
        const status = err.response?.status;
        console.log(`[callGroq] model=${model} gagal (Status: ${status}). Pindah kombinasi...`);
        continue;
      }
    }
  }
  throw lastErr || new Error('Semua kombinasi Groq gagal.');
}
...
const SYSTEM_PROMPT = [
  ...
  // --- Gaya menjawab ---
  'Berpikir step-by-step secara internal sebelum menjawab pertanyaan complexes, tapi tampilkan ke user hanya hasil akhir yang rapi dan actionable — jangan tampilkan "chain of thought" mentah.',
...
async function getBinanceTicker(symbolPair) {
  ...
  try {
    const { data } = await axios.get('https://api.binance.com/api/v3/ticker/24hr', {
      params: { symbol: symbolPair.toUpperCase() },
      timeout: 8000,
    });
    return {
      symbol: data.symbol,
...
async function commitGitHubFile(owner, repo, filePath, newContent, commitMessage, sha, branch) {
  if (!GITHUB_TOKEN) {
    const e = new Error('GITHUB_TOKEN belum di-set.');
...
// ==========================================
// ROUTER UTAMA: Tanya AI dengan Google Search Grounding & Fallback
// ==========================================
async function askGemini(question, model) {
  ...
  // ROUTER: 1. Jalankan via Groq Terlebih Dahulu (Timeout 30s)
  if (GROQ_KEYS.length > 0) {
    try {
      const result = await callGroq(finalQuestion);
      return { text: result.text, sources: result.sources };
    } catch (err) {
      console.log(`[Router] Groq gagal. Otomatis jatuh ke provider Gemini... Error:`, err.message);
    }
  }

  // ROUTER: 2. Fallback ke Gemini (Timeout 45s)
  const body = {
  ...
  const data = await callGemini(modelsToTry, body, 45000);
...
function friendlyError(err) {
  ...
  const failedModel = err.geminiModel || GEMINI_MODEL;
  console.error('[ai] Gagal:', status, apiMsg);

  if (status === 429) return '⚠️ Rate limit tercapai. Coba lagi sebentar lagi.';
  if (status === 503 || status === 500) return '⚠️ Server sedang sibuk/overload. Coba lagi beberapa saat lagi.';
  if (status === 404) return `⚠️ Model "${failedModel}" tidak ditemukan atau tidak aktif.`;
