...
// ============================================================
// REGISTRY PROVIDER
// Setiap provider menerima { question, history, isDMOwner } di buildRequest.
// isDMOwner = true → system prompt coding mendalam aktif di SEMUA provider (Groq/Gemini/OpenAI).
// History selalu dikirim ke provider manapun yang aktif — konteks tetap terjaga saat ganti model/API.
// ============================================================
const PROVIDERS = {
  groq: {
    keys: GROQ_KEYS,
  ...
  gemini: {
    ...
    buildRequest(model, key, { question, history = [], isDMOwner = false }) {
      ...
      // Wajib sanitasi: Gemini error 400 kalau ada dua role sama berturut-turut.
      const cleanHistory = sanitizeHistoryForGemini(history);
...
//                Konteks tetap terjaga meskipun Groq → Gemini → OpenAI roll.
