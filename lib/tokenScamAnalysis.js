...
async function searchWeb(query) {
  try {
    ...
    const snippets = [];
    // Ekstrak snippet hasil pencarian
    const snippetRe = /class="result__snippet"[^>]*>([sS]*?)</a>/g;
    let m;
    while ((m = snippetRe.exec(data)) !== null && snippets.length < 5) {
      const text = m[1]
        .replace(/<[^>]+>/g, '')
    ...
    const titleRe = /class="result__a"[^>]*>([sS]*?)</a>/g;
    while ((m = titleRe.exec(data)) !== null && snippets.length < 8) {
...
async function collect(addresses, chain) {
  ...
  for (const addr of addresses.slice(0,2)) {
    ...
    lines.push(`| isVerified() read | ${isVerified!=null?'✅ = '+isVerified:'❌ tidak tersedia'} |`);
...
[SALIN VERBATIM dari tabel [DATA AVAILABILITY] di atas. Kolom "Status" harus sama persis — tidak boleh diganti kata-katanya. Jika LP Lock check menunjukkan ❓, tulis ❓ dengan teks aslinya.]
