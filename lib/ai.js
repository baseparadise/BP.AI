...
async function callProviderOnce(providerId, opts, timeout, rounds = 3) {
  ...
  let lastErr;
  // Jika 413 (payload terlalu besar), skip seluruh provider —
  // beda model/key tidak akan membantu karena ukuran request tidak berubah.
  let requestTooLarge = false;

  for (let round = 0; round < rounds; round++) {
    if (requestTooLarge) break;

    // Urutan rotasi: key1 → semua model, key2 → semua model, dst.
    // Tiap model punya rate-limit bucket sendiri per key,
    // jadi model2 bisa jalan walau model1 kena 429 di key yang sama.
    const startKeyIdx = keyCursors[providerId] % provider.keys.length;
    for (let ki = 0; ki < provider.keys.length; ki++) {
      if (requestTooLarge) break;
      const keyIdx = (startKeyIdx + ki) % provider.keys.length;
      ...
      for (const model of provider.models) {
        ...
        try {
          ...
          keyCursors[providerId] = keyIdx + 1; // mulai dari key berikutnya di request selanjutnya
          ...
          // 413: payload terlalu besar → skip seluruh provider
          if (status === 413) {
            console.log(`[${providerId}] key#${keyIdx} model=${model} 413 (payload terlalu besar), skip provider ini.`);
