...
const readAllData     = async (a,c) => { const r = await callWithFallback(c,a,'0x773a5096'); return r ? decAllData(r) : null; };
const readIsVerified  = async (a,c) => { const r = await callWithFallback(c,a,'0xe8d5ce15'); return r!=null ? decBool(r) : null; };
...
async function getLpLockInfo(pairAddress, chain, clankerData, bestPair, tokenAddr) {
  ...
  if (pairAddress && !result.lpLocked) {
    const padAddr = (a) => '000000000000000000000000' + a.replace('0x','').toLowerCase();
    ...
    if (lpHolders.length === 0) {
      ...
      // Time-lock detection: if result.lpLocked still false, check top holder for timelock contract
...
async function offChainIntel(sym, name) {
  ...
  const [r1, r2, r3] = await Promise.all([
    ...
    searchWeb(`"${sym}" crypto pump dump CEX investigation 2024 2025 2026`),
  ]);
...
function scoreFlags({ gecko, dex, src, abi, creator, holders, deployer, allData,
  ...
  // isVerified() hanya bermakna jika token adalah Clanker/Doppler (allData terkonfirmasi)
  // Token non-Clanker yang kebetulan punya fungsi ini tidak relevan untuk flag CLANKER
  if (isVerified===false && allData)
    F.push({c:'CLANKER', m:'isVerified() = false — Clanker/Doppler belum verifikasi token ini'});
  ...
  if (lp) {
    if (lp.lpLockMethod === 'none')
      F.push({c:'LP-LOCK', m:'LP token TIDAK dikunci (terbukti dari on-chain check) — dev bisa tarik likuiditas kapan saja'});
    if (lp.lpLockMethod === 'unconfirmed' || lp.lpLockMethod === 'no_apikey')
      F.push({c:'LP-LOCK', m:`LP lock tidak dapat dikonfirmasi (${lp.lpLockMethod}) — data tidak cukup untuk verifikasi`});
...
async function collect(addresses, chain) {
  ...
  lines.push(`[CONFIG] Chain: ${cfg.name||chain} | ChainID: ${cfg.cid||'N/A'} | Etherscan V2: ${hasKey?'✅ key ada':'❌ tidak ada'} | Public RPC: ${hasRpc?'✅ fallback aktif':'❌ tidak tersedia'}`);
  ...
  for (const addr of addresses.slice(0,2)) {
    ...
    lines.push('| Data Point | Status |');
    lines.push('|---|---|');
    lines.push(`| GeckoTerminal (market data) | ${gecko?'✅ tersedia':'❌ tidak ditemukan'} |`);
    lines.push(`| DexScreener (pool info) | ${bestPair?'✅ tersedia':'❌ tidak ditemukan'} |`);
    lines.push(`| Contract source (verified) | ${si&&si.ABI!=='Contract source code not verified'?'✅ tersedia':'❌ tidak tersedia'} |`);
    lines.push(`| ABI functions | ${abi?.length?'✅ tersedia ('+abi.length+' fungsi)':'❌ tidak tersedia'} |`);
    lines.push(`| allData() (Clanker/Doppler) | ${allData?'✅ tersedia':'❌ tidak tersedia / bukan Clanker'} |`);
    lines.push(`| LP Lock check (Bankr) | ${lpStatus} |`);
    lines.push(`| Clanker API | ${clankerData?'✅ tersedia ('+clankerData.recipientCount+' recipient)':'❌ tidak tersedia / bukan token Clanker'} |`);
    lines.push(`| isVerified() read | ${isVerified===true?'✅ = true':isVerified===false?'❌ = false (fungsi ada, return false)':'❌ tidak tersedia'} |`);
    lines.push(`| Deployer info / txlist | ${creator||deployer?'✅ tersedia':'❌ tidak tersedia'} |`);
    lines.push(`| Top holders / per-holder buy | ${holders?.length?'✅ tersedia ('+holders.length+' holder)':'❌ tidak tersedia'} |`);
    lines.push(`| Vault / Airdrop extensions | ${holderMeta.hasVault||holderMeta.hasAirdrop?'✅ terdeteksi':'❌ tidak ada'} |`);
    lines.push(`| OFT detection | ${oftFlag?'✅ YA (LayerZero OFT)':'✅ bukan OFT'} |`);
    lines.push(`| Off-chain intel (ZachXBT, CEX) | ${offChain?.hasResults?'✅ tersedia ('+([offChain.zachXBT&&'ZachXBT',offChain.cexInv&&'CEX',offChain.rugged&&'rug-kw'].filter(Boolean).join(', ')||'tidak ada flag')+')':'❌ tidak ada coverage publik ditemukan'} |`);
...
- **Tabel hanya untuk data yang PUNYA NILAI.** Baris "tidak tersedia" berulang membuat laporan tidak terbaca.
...
---
**📋 Ketersediaan Data**
[SALIN VERBATIM tabel [DATA AVAILABILITY] — jangan ubah kata-kata di kolom Status]
