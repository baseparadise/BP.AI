...
const PROVIDERS = {
  ...
  conduit:{
    ...
    parseResponse(d){const c=d?.choices?.[0];if(c?.finish_reason==='length')console.warn('[conduit] ⚠️ Terpotong.');let raw=c?.message?.content||c?.message?.reasoning_content||'';raw=raw.replace(/<think>[sS]*?</think>/gi,'').trim();return{text:raw||'Tidak ada jawaban.',sources:[]};},
  },
};
