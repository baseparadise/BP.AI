...
// === CONDUIT — OpenAI-compatible proxy: Claude 4, GPT-5, Gemini 3 dll ===
// Set CONDUIT_API_KEY di Railway. Opsional: CONDUIT_MODELS=model1,model2
...
const PROVIDER_ORDER=parseEnvList(process.env.AI_PROVIDER_ORDER,['conduit','groq','gemini','openai']);
...
const PROVIDERS = {
  ...
  openai: {
    ...
    parseResponse(data) {
      ...
      return{text:choice?.message?.content||'Tidak ada jawaban.',sources:[]};
    },
  },
  conduit:{
    keys:CONDUIT_KEYS,models:CONDUIT_MODELS,supportsSearch:false,tokenLimit:PROVIDER_TOKEN_LIMITS.conduit,
    buildRequest(model,key,{question,history=[],isDMOwner=false}){
      const sys=isDMOwner?`${SYSTEM_PROMPT_BASE}${SYSTEM_PROMPT_DM_CODING}\n\n${SYSTEM_PROMPT_NOSEARCH_ADDENDUM}`:`${SYSTEM_PROMPT_BASE} ${SYSTEM_PROMPT_NOSEARCH_ADDENDUM}`;
      const{history:h,dropped}=truncateHistoryToFit(history,sys,question,PROVIDER_TOKEN_LIMITS.conduit);
      if(dropped>0)console.log(`[conduit] ⚠️ History dipotong ${dropped} pasang.`);
      return{url:'https://conduit.ozdoev.net/api/v1/chat/completions',headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},
        body:{model,max_tokens:isDMOwner?8192:700,messages:[{role:'system',content:sys},...h.map(x=>({role:x.role==='assistant'?'assistant':'user',content:x.content})),{role:'user',content:question}]}};
    },
    parseResponse(d){const c=d?.choices?.[0];if(c?.finish_reason==='length')console.warn('[conduit] ⚠️ Terpotong.');let raw=c?.message?.content||c?.message?.reasoning_content||'';raw=raw.replace(new RegExp('<think>[\\s\\S]*?<\\/think>','gi'),'').trim();return{text:raw||'Tidak ada jawaban.',sources:[]};},
  },
};

{
  ...
  for(const _id of['groq','gemini','openai','conduit']){
...
async function callProviderOnce(providerId, opts, timeout) {
  ...
  // Setiap chat baru selalu mulai dari key#0.
  // key#0 → coba semua model → kalau semua gagal → key#1 → semua model → dst.
  // Baru pindah provider (groq/gemini/openai) setelah SEMUA key provider ini habis.
  for (let keyIdx = 0; keyIdx < provider.keys.length; keyIdx++) {
    const key = provider.keys[keyIdx];
    let keyGotAnySuccess = false;
    let keyAllBlocked = true;
    ...
    for (const model of provider.models) {
      ...
      try {
        ...
        // Deteksi meta-error dari AI (Claude mengembalikan pesan internal sebagai teks)
        if (isAIMetaError(parsed.text)) {
          console.warn(`[${providerId}] ⚠️ key#${keyIdx} model=${model} meta-error — teks: "${parsed.text.slice(0,120)}", retry ke provider berikutnya...`);
        ...
        if (status === 400) {
          console.log(`[${providerId}] key#${keyIdx} model=${model} 400 (format tidak didukung), blokir permanen. Hapus dari env.`);
          blockedModels.add(`${providerId}::${model}`);
          continue;
        }
        if (status === 404) {
          console.log(`[${providerId}] key#${keyIdx} model=${model} 404 (model tidak ditemukan), blokir permanen.`);
          blockedModels.add(`${providerId}::${model}`);
          continue;
        }
        if (RETRYABLE_STATUS.includes(status)) {
          console.log(`[${providerId}] key#${keyIdx} model=${model} ${status} (limit/overload), coba model berikutnya...`);
        } else if (!err.response || ['ECONNABORTED', 'ETIMEDOUT'].includes(err.code)) {
          console.log(`[${providerId}] key#${keyIdx} model=${model} timeout/network, coba model berikutnya...`);
        } else {
          console.log(`[${providerId}] key#${keyIdx} model=${model} status ${status} (${apiCode || ''}), coba model berikutnya...`);
        }
      }
    }

    if (!keyGotAnySuccess && !keyAllBlocked) {
      const nextKey = keyIdx + 1 < provider.keys.length ? `key#${keyIdx + 1}` : 'tidak ada key lagi';
      console.log(`[${providerId}] key#${keyIdx} habis semua model, pindah ke ${nextKey}...`);
    }
  }
  if (!lastErr) {
...
// ============================================================
async function askGemini(question, history = [], isDMOwner = false, skipProviders = []) {
  const extraContext = await buildRealtimeContext(question);
  const finalQuestion = question + extraContext;
  // Hitung provider order efektif: buang yang di-skip (misal conduit di-off owner)
  const effectiveOrder = skipProviders.length
    ? PROVIDER_ORDER.filter((p) => !skipProviders.includes(p))
    : null;
  // Timeout 300 detik — beri AI waktu berpikir untuk analisis mendalam
  const result = await callAI({ question: finalQuestion, history, isDMOwner }, 300000, effectiveOrder);
