...
function zerionGet(path, params) {
  return new Promise((resolve, reject) => {
    ...
    // FIX: Jangan encode bracket [] pada key — Zerion pakai filter[trash] bukan filter%5Btrash%5D
    ...
    const req = https.request(opts, (res) => {
      ...
      res.on('end', () => {
        // FIX: Tangani redirect 3xx — jangan coba parse HTML sebagai JSON
        if (res.statusCode >= 300 && res.statusCode < 400) {
          const loc = res.headers.location || '(no location)';
          return reject(new Error('Zerion API redirect ke: ' + loc + ' — cek ZERION_API_KEY'));
        }
        ...
        if (!ct.includes('application/json') && !ct.includes('application/vnd.api+json')) {
          return reject(new Error(
            ...
            'Kemungkinan API key salah atau request diblokir.'
          ));
        }
...
async function fetchZerionPortfolio(address) {
  ...
  const topPos = positions
    .filter(p => {
      ...
      // Tampilkan jika ada nilai USD, atau ada quantity walau price belum ada
  ...
  if (topPos.length) {
    ...
    topPos.forEach((pos) => {
      ...
      // Chain dari relationships atau dari implementations
