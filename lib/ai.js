    ...
    for (const model of modelList) {
      ...
      for (let i = 0; i < API_KEYS.length; i++) {
        ...
        try {
          ...
          lastErr = err;
          lastErr.geminiModel = model; // simpan model yang sebenarnya gagal, dipakai friendlyError()
...
function friendlyError(err) {
  ...
  const failedModel = err.geminiModel || GEMINI_MODEL; // model asli yang gagal (lihat callGemini), fallback ke default
  ...
  if (status === 404) return `⚠️ Model "${failedModel}" tidak ditemukan/sudah dipensiunkan. Set env GEMINI_MODEL ke model yang aktif.`;
