...
const LP_LOCKERS = {
  eth: new Map([
    ...
    ['0x7a250d5630b4cf539739df2c5dacb4c659f2488d', 'UniswapV2Router'], // not locker but common FP
  ]),
  bsc: new Map([
  ...
  polygon: new Map([
    ...
    ['0x7a250d5630b4cf539739df2c5dacb4c659f2488d', 'QuickSwap Router'],
  ]),
  arbitrum: new Map([
  ...
  base: new Map([
    ...
    ['0x1234556b5443a1de3cfa44b45e3f070c5e3e92b9', 'Clanker v4 Locker'], // placeholder — real addrs may vary
  ]),
};
...
async function getLpLockInfo(pairAddress, chain, clankerData) {
  const result = {
    lpLocked:      false,
    lpLockerName:  null,
    lpLockerAddr:  null,
    lpLockedPct:   null,
    lpLockMethod:  null,   // 'clanker_native' | 'lp_holder' | 'none'
    vaultLock:     null,   // { pct, unlockDate, daysLeft, amount }
    airdropLock:   null,   // { pct, unlockDate, daysLeft, amount }
    notes:         [],
  };

  const supplyTotal = clankerData?.vault?.amount || clankerData?.airdrop?.amount
    ? BigInt(clankerData?.vault?.amount || '0') + BigInt(clankerData?.airdrop?.amount || '0')
    : null;

  // ── Layer 1: Clanker native LP locker
  if (clankerData?.locker_address) {
    result.lpLocked     = true;
    result.lpLockerAddr = clankerData.locker_address;
    result.lpLockerName = 'Clanker v4 Native LP Locker';
    result.lpLockMethod = 'clanker_native';
    result.notes.push('LP di-lock secara native oleh Clanker v4 protocol di locker_address');
  }

  // ── Layer 2: Vault supply lock (token supply, bukan LP)
  if (clankerData?.vault?.lockup) {
    ...
    // 100B supply = 100_000_000_000 * 1e18
  ...
  // ── Layer 3: Etherscan LP tokenholderlist (non-Clanker or additional confirmation)
  if (pairAddress && ESCAN_KEY) {
    ...
    if (Array.isArray(lpHolders)) {
      const lockerMap = LP_LOCKERS[chain] || new Map();
      // Also add dynamic Clanker locker if known
      if (clankerData?.locker_address) {
      ...
      for (const h of lpHolders) {
        ...
        if (lockerMap.has(addr)) {
          const totalLp = parseFloat(h.TotalSupply || '1');
          const heldLp  = parseFloat(h.TokenHolderQuantity || '0');
          const lockedPct = totalLp ? (heldLp / totalLp * 100).toFixed(1) : null;
          if (!result.lpLocked) {
            result.lpLocked     = true;
          ...
          result.notes.push(`LP holder check: ${lockerMap.get(addr)} pegang ${lockedPct}% LP token`);
          break;
        }
      }
      // If no locker found among top 10 LP holders, note it
      if (!result.lpLocked && lpHolders.length > 0) {
        result.notes.push('Top 10 LP token holders tidak ada yang cocok dengan known locker contracts');
        result.lpLockMethod = 'none';
      }
    }
  }

  // Fallback note if no LP check was possible
  if (!result.lpLockerAddr && !pairAddress) {
    result.notes.push('LP pair address tidak ditemukan — tidak bisa cek LP lock');
  }
...
async function collect(addresses, chain) {
  ...
  for (const addr of addresses.slice(0,2)) {
    ...
    const [allData, isVerified, totalSupply, owner, deployer, offChain, lpLock] = await Promise.all([
      ...
      getLpLockInfo(bestPair?.pairAddress || null, chain, clankerData),
    ]);
    ...
    if (!lpLock) {
      ...
      // LP lock status
      if (lpLock.lpLocked) {
        lines.push(`  LP Status:     ✅ LOCKED — ${lpLock.lpLockerName}`);
        lines.push(`  Locker addr:   ${lpLock.lpLockerAddr} | ${cfg.ex}/address/${lpLock.lpLockerAddr}`);
        if (lpLock.lpLockedPct) lines.push(`  LP dikunci:    ${lpLock.lpLockedPct}% dari total LP token`);
      } else if (lpLock.lpLockMethod === 'none') {
        lines.push('  LP Status:     ❌ TIDAK DIKUNCI — dev bisa tarik likuiditas kapan saja');
      } else {
        lines.push('  LP Status:     ❓ Tidak bisa dikonfirmasi (perlu ETHERSCAN_API_KEY atau non-EVM)');
      }
      // Vault supply lock
      if (lpLock.vaultLock) {
...
### LP Lock Detection — 3 Layer
- **Layer 1 (Clanker)**: `locker_address` dari Clanker API = LP di-lock secara native → aman
- **Layer 2 (Supply lock)**: `extensions.vault.lockup` + `extensions.airdrop.lockup` = tanggal unlock supply token team
- **Layer 3 (LP token holders)**: cek apakah LP token dipegang locker contract (UNCX/Team.Finance/PinkSale/Mudra)
- **Red flag**: `lpLockMethod = 'none'` = tidak ada locker yang pegang LP = dev BISA tarik likuiditas kapan saja
- **Warning**: vault/airdrop lock yang sudah expired = team bisa jual token yang sebelumnya terkunci
