...
const GEMINI_MODELS = (process.env.GEMINI_MODEL || 'gemini-1.5-flash').split(',').map(m => m.trim());
...
console.log(`[callGemini] ${API_KEYS.length} API key dimuat dari GEMINI_API_KEYS`);
  ...
  for (let round = 0; round < rounds; round++) {
    ...
    for (let i = 0; i < API_KEYS.length; i++) {
      ...
      try {
        ...
        // Timeout / network error (tidak ada response) → coba key/putaran lain
        if (!err.response || err.code === 'ECONNABORTED' || err.code === 'ETIMEDOUT') {
          console.log(`[callGemini] timeout/network error, coba key lain...`);
          continue;
        }
...
async function askGemini(question, modelIndex = 0) {
  ...
  try {
    ...
    const status = err.response?.status;
    if ([429, 500, 503].includes(status) && modelIndex < GEMINI_MODELS.length - 1) {
      console.log(`[askGemini] Model ${model} gagal (${status}), pindah ke model berikutnya...`);
      return askGemini(question, modelIndex + 1);
    }
    throw err;
  }
}
