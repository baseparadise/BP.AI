...
// === OPENAI ===
// PENTING: OpenAI TIDAK punya free tier yang berguna lagi (kredit $5 otomatis sudah dihapus sejak 2025).
// Tanpa billing/prepaid aktif di akun OpenAI, HANYA gpt-3.5-turbo (3 RPM) yang bisa dipanggil — model modern
// (gpt-4o-mini, gpt-5-mini, dst) akan selalu gagal (401/403) tanpa billing. Makanya gpt-3.5-turbo SENGAJA
// ditaruh PALING TERAKHIR di daftar: kalau kamu sudah aktifkan billing, model yang lebih bagus dicoba duluan;
// kalau belum, otomatis jatuh ke gpt-3.5-turbo yang masih jalan gratis.
...
const PROVIDER_ORDER = parseEnvList(process.env.AI_PROVIDER_ORDER, ['groq', 'gemini', 'openai']);
...
const PROVIDERS = {
  ...
  openai: {
    ...
    supportsSearch: false, // Chat Completions biasa tidak ada web grounding bawaan (beda dari Responses API + tool search).
    buildRequest(model, key, { question }) {
      return {
...
// Set untuk menandai kombinasi provider+model yang permanen diblokir (mis. 403 model_permission_blocked_org dari Groq).
// Sekali ditandai, tidak akan dicoba lagi sampai proses restart — supaya tiap request tidak buang attempt percuma.
const blockedModels = new Set(); // entry: `${providerId}::${model}`
      ...
      for (let i = 0; i < provider.keys.length; i++) {
        ...
        try {
          ...
          if (status === 403 && (apiCode === 'model_permission_blocked_org' || /blocked at the organization|permission/i.test(apiMsg))) {
            console.log(`[${providerId}] model=${model} key#${keyIdx} DIBLOKIR org-level (403), skip model ini untuk request berikutnya.`);
            blockedModels.add(`${providerId}::${model}`);
            break; // semua key akan kena 403 yang sama → langsung ke model berikutnya
          }
...
async function askGemini(question) {
  ...
  const result = await callAI({ question: finalQuestion }, 45000);
