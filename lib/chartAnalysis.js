...
// Kraken hanya punya: 1,5,15,30,60,240,1440,10080,21600
...
// FIX: QuickChart tidak support multi-axis (yAxisID) → 400 error.
// Solusi: gunakan single y-axis, volume dinormalisasi ke rentang harga.
async function buildChart(pair, ohlcv, sma20, sma50, bb, interval) {
  ...
  // Normalisasi volume ke 20% bawah rentang harga agar bisa single-axis
  const allPrices = sl.flatMap(c => [c.l, c.h]);
  const minPrice = Math.min(...allPrices);
  const maxPrice = Math.max(...allPrices);
  const priceRange = maxPrice - minPrice || 1;
  const maxVol = Math.max(...sl.map(c => c.v)) || 1;
  // Volume ditampilkan di 20% bawah chart (dari minPrice - 25% ke minPrice - 5%)
  const volBase = minPrice - priceRange * 0.25;
  const volTop  = minPrice - priceRange * 0.05;
  const scaledVol = sl.map(c => volBase + (c.v / maxVol) * (volTop - volBase));
  ...
  const cfg = {
    ...
    data: {
      ...
      datasets: [
        ...
        },
        {
          ...
          label: 'Volume',
          data: scaledVol,
          backgroundColor: sl.map(c => c.c >= c.o ? 'rgba(38,166,154,0.4)' : 'rgba(239,83,80,0.4)'),
          order: 10,
        },
      ],
    },
    options: {
      plugins: {
        ...
        title: {
          display: true,
          text: pair+' | '+interval.toUpperCase()+' — Close / SMA20 / SMA50 / BB',
          color: '#EEE',
  ...
  try {
    ...
    // Log body error sebenarnya dari QuickChart agar mudah debug
    if (e.response && e.response.data) {
      const errBody = Buffer.isBuffer(e.response.data)
        ? e.response.data.toString('utf8')
        : String(e.response.data);
      console.error('[chart] QuickChart error ('+e.response.status+'):', errBody.slice(0, 300));
    }
    throw e;
  }
}
