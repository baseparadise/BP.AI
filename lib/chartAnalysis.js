...
// ─── New chartAnalysis.js ────────────────────────────────────────────────────
const CHART_CODE = `'use strict';
...
async function fetchOHLCV(pair, interval, limit) {
  try { return await fetchBinance(pair, interval, limit); }
    ...
    if (s===451||s===403||s===418) { return await fetchKraken(pair, interval, limit); }
    throw e;
  }
}

function calcSMA(arr,p){return arr.map((_,i)=>i<p-1?null:arr.slice(i-p+1,i+1).reduce((a,b)=>a+b,0)/p);}
function calcEMA(arr,p){const k=2/(p+1);let e=null;const r=[];for(let i=0;i<arr.length;i++){if(i<p-1){r.push(null);continue;}e=e===null?arr.slice(0,p).reduce((a,b)=>a+b,0)/p:arr[i]*k+e*(1-k);r.push(e);}return r;}
function calcRSI(closes,p){p=p||14;const out=new Array(p).fill(null);let ag=0,al=0;for(let i=1;i<=p;i++){const d=closes[i]-closes[i-1];d>0?ag+=d:al-=d;}ag/=p;al/=p;out.push(al===0?100:100-100/(1+ag/al));for(let i=p+1;i<closes.length;i++){const d=closes[i]-closes[i-1],g=d>0?d:0,l=d<0?-d:0;ag=(ag*(p-1)+g)/p;al=(al*(p-1)+l)/p;out.push(al===0?100:100-100/(1+ag/al));}return out;}
function calcBB(closes,p,m){p=p||20;m=m||2;return calcSMA(closes,p).map((mid,i)=>{if(mid===null)return{upper:null,mid:null,lower:null};const s=Math.sqrt(closes.slice(i-p+1,i+1).reduce((a,v)=>a+Math.pow(v-mid,2),0)/p);return{upper:mid+m*s,mid,lower:mid-m*s};});}
function calcMACD(closes){const e12=calcEMA(closes,12),e26=calcEMA(closes,26);const ml=e12.map((v,i)=>v!==null&&e26[i]!==null?v-e26[i]:null);const sl=calcEMA(ml.map(v=>v!==null?v:0),9);return ml.map((v,i)=>({macd:v,signal:sl[i],hist:v!==null&&sl[i]!==null?v-sl[i]:null}));}
...
function analyzeAll(ohlcv,closes,rsi,macdData,sma20,sma50,bb){
  ...
  if(lastMACD&&lastMACD.macd!==null&&lastMACD.signal!==null){
    if(lastMACD.macd>lastMACD.signal){signals.push('🟢 MACD → **Bullish momentum**');bull++;if(prevMACD&&prevMACD.macd<=prevMACD.signal){signals.push('⚡ **MACD Bullish Crossover**!');bull++;}}
    else{signals.push('🔴 MACD → **Bearish momentum**');bear++;if(prevMACD&&prevMACD.macd>=prevMACD.signal){signals.push('⚡ **MACD Bearish Crossover**!');bear++;}}
  }
...
// ─── Canvas Chart Renderer (lokal, tanpa API eksternal) ───────────────────────
async function buildChart(pair, ohlcv, sma20, sma50, bb, interval) {
  const { createCanvas } = require('@napi-rs/canvas');
  const W=900, H=500, PAD={top:45,right:80,bottom:50,left:85};
  const CW=W-PAD.left-PAD.right, CH=H-PAD.top-PAD.bottom;
  const CVOL=80; // tinggi area volume
  const CPH=CH-CVOL-10; // tinggi area harga
  ...
  // skala harga
  const allPrices=[...closes,...s20.filter(Boolean),...s50.filter(Boolean),...bbs.map(b=>b.upper).filter(Boolean),...bbs.map(b=>b.lower).filter(Boolean)];
  const minP=Math.min(...allPrices)*0.998, maxP=Math.max(...allPrices)*1.002;
  const scaleY=v=>(1-(v-minP)/(maxP-minP))*CPH;

  // skala volume
  const vols=sl.map(c=>c.v);
  const maxV=Math.max(...vols)||1;
  const scaleVY=v=>(1-v/maxV)*CVOL;

  // skala X
  const barW=Math.max(2,Math.floor(CW/n)-1);
  const scaleX=i=>i*(CW/n)+(CW/n)*0.5;
  ...
  sl.forEach((c,i)=>{
    const x=scaleX(i);
    const barH=CVOL-scaleVY(c.v);
    const y0=CPH+10+scaleVY(c.v);
    ctx.fillStyle=c.c>=c.o?'rgba(38,166,154,0.5)':'rgba(239,83,80,0.5)';
    ctx.fillRect(x-barW/2,y0,barW,barH);
  });

  // BB bands fill
  ctx.beginPath();
  let started=false;
  bbs.forEach((b,i)=>{if(b.upper===null)return;const x=scaleX(i),y=scaleY(b.upper);started?ctx.lineTo(x,y):ctx.moveTo(x,y);started=true;});
  const validBBS=bbs.filter(b=>b.lower!==null);
  for(let i=validBBS.length-1;i>=0;i--){const idx=bbs.lastIndexOf(validBBS[i]);ctx.lineTo(scaleX(idx),scaleY(validBBS[i].lower));}
  ctx.closePath();
  ctx.fillStyle='rgba(206,147,216,0.07)';
  ctx.fill();

  // BB upper
  drawLine(ctx,bbs.map((b,i)=>b.upper!==null?{x:scaleX(i),y:scaleY(b.upper)}:null).filter(Boolean),'rgba(206,147,216,0.7)',1,[5,5]);
  // BB lower
  drawLine(ctx,bbs.map((b,i)=>b.lower!==null?{x:scaleX(i),y:scaleY(b.lower)}:null).filter(Boolean),'rgba(206,147,216,0.7)',1,[5,5]);
  // SMA50
  drawLine(ctx,s50.map((v,i)=>v!==null?{x:scaleX(i),y:scaleY(v)}:null).filter(Boolean),'#FFD54F',1.5);
  // SMA20
  drawLine(ctx,s20.map((v,i)=>v!==null?{x:scaleX(i),y:scaleY(v)}:null).filter(Boolean),'#4FC3F7',1.5);
  // Price line
  drawLine(ctx,closes.map((v,i)=>({x:scaleX(i),y:scaleY(v)})),'#F7931A',2.5);

  // Price area fill
  ctx.beginPath();
  closes.forEach((v,i)=>{i===0?ctx.moveTo(scaleX(i),scaleY(v)):ctx.lineTo(scaleX(i),scaleY(v));});
  ctx.lineTo(scaleX(n-1),CPH);
  ctx.lineTo(scaleX(0),CPH);
  ctx.closePath();
  const grad=ctx.createLinearGradient(0,0,0,CPH);
  grad.addColorStop(0,'rgba(247,147,26,0.25)');
  grad.addColorStop(1,'rgba(247,147,26,0.01)');
  ctx.fillStyle=grad;
  ctx.fill();
  ...
  ctx.textAlign='right';
  for(let i=0;i<=4;i++){
    const v=minP+(maxP-minP)*(1-i/4);
    const y=PAD.top+CPH*i/4+4;
    const label=v>=1000?'$'+(v/1000).toFixed(1)+'k':'$'+v.toFixed(2);
  ...
  sl.forEach((c,i)=>{
    ...
    const lbl=(interval.includes('d')||interval.includes('w')||interval==='1M')
      ?(d.getMonth()+1)+'/'+(d.getDate())
      :(d.getMonth()+1)+'/'+(d.getDate())+' '+(d.getHours())+'h';
    ctx.fillStyle='#666';
    ctx.font='10px monospace';
    ctx.fillText(lbl,PAD.left+scaleX(i),H-PAD.bottom+16);
  });
  ...
  ctx.textAlign='left';
  ctx.fillText(pair+' | '+interval.toUpperCase(),PAD.left,22);

  // Legend
  const legends=[['─','#F7931A','Close'],['─','#4FC3F7','SMA20'],['─','#FFD54F','SMA50'],['- -','rgba(206,147,216,0.9)','BB']];
  let lx=PAD.left+120;
  legends.forEach(([sym,color,name])=>{
    ctx.fillStyle=color;ctx.font='11px monospace';
    ctx.fillText(sym+' '+name,lx,22);
    lx+=sym.length*7+name.length*7+14;
  });
...
function fmtPrice(v){if(v>=1000)return '$'+v.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2});if(v>=1)return '$'+v.toFixed(4);return '$'+v.toPrecision(5);}
...
async function handleChartCommand(message, symbol, interval) {
  try {
    ...
    try { ohlcv=await fetchOHLCV(pair,interval,120); }
      ...
      const s=e.response&&e.response.status;
      await message.reply(s===400||(e.message&&e.message.includes('tidak tersedia'))
        ?'⚠️ Pasangan **'+pair+'** tidak ditemukan. Coba: btc, eth, sol, dll.'
        :'⚠️ Gagal ambil data **'+pair+'**: '+e.message).catch(()=>{});
      return;
    }
    if(!ohlcv||ohlcv.length<30){await message.reply('⚠️ Data tidak cukup untuk **'+pair+'**.').catch(()=>{});return;}
    ...
    if(patterns.length>0){text+='\\n**🕯️ Pola Candlestick:**\\n'+patterns.map(p=>'• '+p.label+' — '+p.desc).join('\\n')+'\\n';}
    ...
    try { chartBuf=await buildChart(pair,ohlcv,sma20,sma50,bb,interval); }
      ...
      console.error('[chart] canvas render gagal:',e.message);
