...
async function fetchOKX(pair, interval, limit) {
  ...
  const { data } = await axios.get(url, { timeout: 8000 });
  if (!data.data || !data.data.length) throw new Error('OKX: data kosong untuk '+instId);
...
async function fetchBybit(pair, interval, limit) {
  ...
  const url = 'https://api.bybit.com/v5/market/kline?category=spot&symbol='+pair+'&interval='+bi+'&limit='+Math.min(limit,200);
  const { data } = await axios.get(url, { timeout: 8000 });
...
async function fetchKuCoin(pair, interval, limit) {
  ...
  const url = 'https://api.kucoin.com/api/v1/market/candles?type='+type+'&symbol='+instId+'&startAt=1600000000&endAt=2000000000';
  const { data } = await axios.get(url, { timeout: 8000 });
  ...
  for (const src of sources) {
    try {
      ...
      if (result && result.length >= 10) {
        if (src.name !== 'Binance') console.log('[chart] Sumber: '+src.name+' ('+result.length+' candle)');
        return result;
      }
    } catch(e) {
...
async function handleChartCommand(message, symbol, interval, candles) {
  try {
    ...
    const displayN = Math.min(Math.max(parseInt(candles)||60, 10), 200);
    ...
    let text = '**📊 '+pair+' | '+interval.toUpperCase()+' | '+displayN+' candle** '+chgEmoji+' '+chgStr+'\n';
    ...
    let file = null;
    try {
      const buf = await buildChart(pair,ohlcv,sma20,sma50,bb,rsi,macdData,interval,candlePatterns,complexPatterns,displayN);
